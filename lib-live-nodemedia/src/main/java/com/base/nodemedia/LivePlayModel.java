...
public class LivePlayModel extends BaseLiveModel implements NodePlayerDelegate, ILivePlayModel {
    private NodePlayer np;
    private NodePlayerView npv;
    private boolean isClose;
    @Override
    public void onCreate(View playView, int mixBuffer, int maxBuffer) {

        //创建NodePlayer实例
        np = new NodePlayer(mActivity);
         npv = (NodePlayerView) playView;
        if (npv != null) {
            //查询播放视图
            //设置播放视图的渲染器模式,可以使用SurfaceView或TextureView. 默认SurfaceView
            npv.setRenderType(NodePlayerView.RenderType.TEXTUREVIEW);
            //设置视图的内容缩放模式
...
//        mCapturePictureThread.start();
    }

    @Override
    public View getRenderView() {
        return npv.getRenderView();
    }

    @Override
    public void capturePicture(String path) {
//        getBitmap((TextureView) npv.getRenderView());
//        isCapturePicture = true;
//        mCapturePictureThread.wake(this);
    }
...
//        }
    ...
    //    private WaitWakeThread mCapturePictureThread = new WaitWakeThread(new IWaitWakeTask() {
//        @Override
//        public void run() {
//            isCapturePicture = false;
//            SurfaceHolder surfaceHolder = ((SurfaceView) npv.getRenderView()).getHolder();
...
//            npv.getRenderView().draw(cc);
//            File file = new File("/sdcard/sss" + ".jpg");
